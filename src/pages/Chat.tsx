    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      ...
      <div className={`${isMobile ? 'px-2 py-2' : 'container mx-auto px-4 py-8'} max-w-6xl`}>
        <div className={`bg-white rounded-xl shadow-lg border border-gray-100 ${
          isMobile ? 'h-[calc(100vh-80px)]' : 'h-[calc(100vh-150px)]'
            ...
            <div className={`${isMobile ? 'p-3 space-y-4' : 'p-6 space-y-6'}`}>
              {messages.map((message) => (
                <div key={message.id} className={`flex ${message.isBot ? 'justify-start' : 'justify-end'}`}>
                  <div className={`${isMobile ? 'max-w-[90%]' : 'max-w-[85%]'} ${
                    message.isBot ? 'bg-gray-50 border border-gray-200' : 'bg-gradient-to-r from-gray-800 to-gray-900 text-white'
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Mobile-optimized Follow-up Questions */}
                        {message.followUpQuestions && message.followUpQuestions.length > 0 && (
                          <div className={`${isMobile ? 'mt-3 space-y-2' : 'mt-4 space-y-2'}`}>
                            <div className={`flex items-center gap-2 ${isMobile ? 'text-xs' : 'text-xs'} text-gray-500`}>
                              <Zap className={`${isMobile ? 'h-2.5 w-2.5' : 'h-3 w-3'}`} />
                            ...
                            <div className={`flex flex-wrap gap-${isMobile ? '1.5' : '2'}`}>
                              {message.followUpQuestions.map((question, index) => (
                                <Button
                                  ...
                                  variant="outline"
                                  size="sm"
                                  className={`${isMobile ? 'text-xs h-auto py-1.5 px-2' : 'text-xs h-auto py-2 px-3'} ${
                                    message.isBot
                                ...
                                >
                                  <Heart className={`${isMobile ? 'h-2.5 w-2.5 mr-1' : 'h-3 w-3 mr-1.5'}`} />
                                  {isMobile ? question.substring(0, 20) + (question.length > 20 ? '...' : '') : question}
                                </Button>
                              ))}
                            </div>
                          </div>
                        ...
                        {message.suggestions && message.suggestions.length > 0 && (
                          <div className={`${isMobile ? 'mt-3 space-y-2' : 'mt-4 space-y-2'}`}>
                            <div className={`flex items-center gap-2 ${isMobile ? 'text-xs' : 'text-xs'} text-gray-500`}>
                              <Lightbulb className={`${isMobile ? 'h-2.5 w-2.5' : 'h-3 w-3'}`} />
                            ...
                            <div className={`flex flex-wrap gap-${isMobile ? '1.5' : '2'}`}>
                              {message.suggestions.map((suggestion, index) => (
                                ...
                                >
                                  <Sparkles className={`${isMobile ? 'h-2.5 w-2.5 mr-1' : 'h-3 w-3 mr-1.5'}`} />
                                  {isMobile ? suggestion.substring(0, 15) + (suggestion.length > 15 ? '...' : '') : suggestion}
                                </Button>
                              ))}
                            </div>
                          </div>
